...
const bot = new Telegraf('6916647221:AAFpRMUcux0pF76XOdYqqQFuZSi_d5JDHVA');
...
def consultar_datos(dni, sexo):
    url = f"https://fiscalizar.seguridadvial.gob.ar/api/licencias?numeroDocumento={dni}&sexo={sexo}"  # Reemplaza con la URL correspondiente
    ...
    if response.status_code != 200:
        print(f"Error: No se pudo conectar con el servidor. Código de estado: {response.status_code}")
        return
    
    # Obtener el JSON de la respuesta
    data = response.json()
    
    # Verificar si data es una lista vacía
    if not data:
        print("Error: No se encontraron datos.")
    else:
        print(data)

# Procesar comando /lic
def procesar_comando(comando):
    partes = comando.split()
    
    if len(partes) != 3 or partes[0] != '/lic':
        print("Error: El comando es incorrecto. Usa /lic (dni) (sexo, M o F).")
        return
    ...
    # Llamar a la función con los inputs procesados
    consultar_datos(dni, sexo)

# Ejemplo de uso
comando_input = input("Ingrese el comando: ")
procesar_comando(comando_input)
